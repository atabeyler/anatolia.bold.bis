...
| `ALLOW_MOCK_BIOMETRICS` | In production, `BIOMETRIC_PROVIDER` defaults to (and today can only be) the non-biometric `MockBiometricProvider` — see the row below. Production refuses to start with it unless this is explicitly set to `true`, a conscious acknowledgment that the deployment is not yet doing real face comparison. Not required outside production. |
...
| `BIOMETRIC_PROVIDER` | Selects the `BiometricProvider` implementation (`server/src/biometric.rs`). Only `"mock"` exists today (the default) — any other value is a hard startup failure, in every environment, until a real provider ships (see `docs/ROADMAP.md` Phase 4). See `ALLOW_MOCK_BIOMETRICS` above for the additional production requirement. |
...
| `ENABLE_SELF_PING` | Set to `false` to disable the periodic self-ping that keeps a Render free-plan instance from cold-starting after ~15 minutes idle (see `main.rs`). Has no effect outside Render (`RENDER_EXTERNAL_URL` unset). | No |
| `MFA_REQUIRED_ROLES` | Comma-separated role list that must have TOTP MFA enrolled before login can complete (see `server/src/mfa.rs`, `API.md`). Defaults to `SYSTEM_ADMIN,SECURITY_ADMIN,REVIEWER`. Set to an empty value (`MFA_REQUIRED_ROLES=`) to disable mandatory MFA entirely — voluntary enrollment remains available to every role either way. | No |
