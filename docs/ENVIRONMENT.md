...
| `ENABLE_SELF_PING` | Set to `false` to disable the periodic self-ping that keeps a Render free-plan instance from cold-starting after ~15 minutes idle (see `main.rs`). Has no effect outside Render (`RENDER_EXTERNAL_URL` unset). | No | No |
